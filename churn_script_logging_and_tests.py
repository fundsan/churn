...
from churn_library import encoder_helper, perform_feature_engineering, train_models
...
def test_eda():
    ...
    try:
        perform_eda(test_df)
        logging.info(
            "Testing eda: directory to save analysis images  SUCCESSFULLY")
    except FileNotFoundError as err:
        logging.error("Testing eda: The directory to save file wasn't found")
        raise err
    try:
        ...
        logging.info(
            "Testing eda: exploratory data analysis images saved SUCCESSFULLY")
    except AssertionError as err:
...
def test_encoder_helper():
    ...
    test_df = test_df[['Attrition_Flag', 'Customer_Age',
                       'Dependent_count', 'Gender', 'Marital_Status']]
    categorical_test = ['Gender', 'Marital_Status']
    try:
        assert set(encoder_helper(test_df, categorical_test).columns.tolist()) == set([
            'Attrition_Flag', 'Customer_Age', 'Dependent_count', 'Gender',
            'Marital_Status', 'Churn', 'Gender_Churn', 'Marital_Status_Churn'])
        logging.info(
            "Testing encoder_helper: SUCCESSFULLY Created new encoder columns")
    except AssertionError as err:
        logging.error("Testing encoder_helper: expected " + str([
            'Attrition_Flag', 'Customer_Age', 'Dependent_count', 'Gender',
            'Marital_Status', 'Gender_Churn', 'Marital_Status_Churn'
        ]) + " for columns but got " + str(encoder_helper(test_df,
...
def test_perform_feature_engineering():
    ...
    x_train, x_test, y_train, y_test = perform_feature_engineering(
        test_df, "Churn")
    try:
        assert set(x_train.columns.tolist()) == set([
        ...
        logging.info(
            "Testing perform_feature_engineering: SUCCESSFULLY Created sets with correct columns")
    except AssertionError as err:
        logging.error(
            "Testing perform_feature_engineering: expected "
            + str(['Customer_Age', 'Dependent_count', 'Months_on_book',
                  'Total_Relationship_Count', 'Months_Inactive_12_mon',
                   'Contacts_Count_12_mon', 'Credit_Limit', 'Total_Revolving_Bal',
                   'Avg_Open_To_Buy', 'Total_Amt_Chng_Q4_Q1', 'Total_Trans_Amt',
                   'Total_Trans_Ct', 'Total_Ct_Chng_Q4_Q1', 'Avg_Utilization_Ratio',
                   'Gender_Churn', 'Education_Level_Churn', 'Marital_Status_Churn',
                   'Income_Category_Churn', 'Card_Category_Churn']) +
            " for columns but got " + str(x_train.columns))
        raise err
