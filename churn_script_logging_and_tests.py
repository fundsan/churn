...
def test_eda():
    ...
    try:
        ...
        assert os.path.isfile("./images/eda/Gender_dist.png")
...
def test_encoder_helper():
    ...
    try:
        assert set(encoder_helper(test_df,
                                  categorical_test).columns.tolist()) == set(['Attrition_Flag',
                                                                              'Customer_Age',
                                                                              'Dependent_count',
                                                                              'Gender',
                                                                              'Marital_Status',
        ...
        logging.error(
            "Testing encoder_helper: expected " + str(
                [
                    'Attrition_Flag',
                    'Customer_Age',
                    'Dependent_count',
                    ...
                    'Marital_Status',
                    'Gender_Churn',
                    'Marital_Status_Churn']) + " for columns but got " + str(
                encoder_helper(
                    test_df,
    ...
    try:
        assert set(
            ...
            [
                ...
                'Card_Category_Churn'])
        logging.info(
            "Testing perform_feature_engineering: SUCCESSFULLY Created Training and Test set with correct columns")
    except AssertionError as err:
        logging.error(
            "Testing perform_feature_engineering: expected " + str(
                [
                    'Customer_Age',
                    'Dependent_count',
                    'Months_on_book',
                    ...
                    'Marital_Status_Churn',
                    'Income_Category_Churn',
                    'Card_Category_Churn']) + " for columns but got " + str(
                X_train.columns))
        raise err
